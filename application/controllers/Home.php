...
class Home extends CI_Controller {
	...
	public function tambahpelamar(){
        ...
        if($datakar->num_rows()>0){
            $result=array("result"=>"error","title"=>"Gagal","msg"=>"Alamat Email ".$this->input->post('email')." telah digunakan, input email yg lain.");
    ...
    public function ResetPass($key=""){
        ...
        if($datediff>86400 || count($var)==0 || $key=="" || !isset($var[0]) || !isset($var[1])){
            ...
            if($sel->num_rows()>0){
                ...
                $result=array("result"=>"error","title"=>"Gagal","msg"=>"Link tidak berlaku, silahkan ulangi.. ".$var[0]." ".$var[1]);
                $this->session->set_flashdata($result);
                redirect("Home/forgotPassword");
            }
        }
	...
	public function prosesLogin() {
        ...
        $whereapplicant = array(
                'u.username' => $user_name,
                'u.password' =>$password,
                'u.status' =>1,
                'u.id_jenis_user'=>5
        );
        $applicant = $this->My_model->cek_loginapplicant($whereapplicant)->num_rows();
        $dtapplicant = $this->My_model->cek_loginapplicant($whereapplicant)->row();
		...
		if($cek > 0){
            if(strtolower($dt->password)=="ggi") {
                 $result=array("result"=>"error","title"=>"Gagal","nik"=>$dt->nik,"msgpwd"=>"Gagal Masukan Email Anda","nik"=>$dt->nik,"username"=>$user_name,"password"=>$password);
                            $this->session->set_flashdata($result);
                           // $data_session['username']=$password;
                             redirect("home/login"); 
            }
            else{
            ...
            }elseif (in_array('5', $explodejenis)) {
                $where = array('a.id_jenis_user',5);
            }
            ...
            if ($dt->status_karyawan == '5') {
                $result=array("result"=>"error","title"=>"Gagal","msg"=>"Anda Sudah Resign");
                $this->session->set_flashdata($result);
                redirect("home/login");
            }
            if($dt->email!=""){
                ...
                if (count($explodejenis) > 1) {
                 ...
                 $result=array("result"=>"error","title"=>"Gagal","nik"=>$dt->nik,"msge"=>"Gagal Masukan Email Anda","nik"=>$dt->nik,"username"=>$user_name,"password"=>$password);
                        $this->session->set_flashdata($result);
                        $data_session['password']=$password;
                       //s $this->session->set_flashdata($data_session);
                         redirect("home/login"); 
            }
        }
            ...
            if ($applicant > 0) {
                ...
                $data_session_applicant = array('username'=>$user_name,
                                        'id_pelamar'=>$dtapplicant->id_pelamar,
                                        'jenis_user'=>$dtapplicant->id_jenis_user,
                                        'nama_lengkap'=>$dtapplicant->nama_lengkap,
                                        'employee_login_id'=>$dtapplicant->id_user,
                                        'nikktp' => '0',
                                        'id_jabatan' =>0,
                                        'user_type'=>5,
                                        'nik' => $dtapplicant->nik,
                ...
                $this->session->set_userdata($data_session_applicant);
                if ($data_session_applicant['user_type']==5) {
                   redirect("Aplicant/recruitment");
                    // print_r($data_session_applicant);
                    // echo "masuk";
                }
                ...
                if($password != $dt->password){
                    $result=array("result"=>"error","title"=>"Gagal","msg"=>"Maaf password anda salah ");
                    $this->session->set_flashdata($result);
                    redirect("home/login");
                }else{
                    $result=array("result"=>"error","title"=>"Gagal","msg"=>"User ".$user_name." belum terdaftar");
                    $this->session->set_flashdata($result);
                    redirect("home/login");
                }
    ...
    public function updatepassword(){
        ...
        $this->My_model->updatepassword($data,$nik);
